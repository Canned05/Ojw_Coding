...
def setup_gemstone():
    ...
    diamond_price, diamond_speed = 600, 2


    #작은 금
    small_gold = Gemstone(gemstone_images[0], (200,380), small_gold_price, small_gold_speed) # 0번째 이미지를 (200,380) 위치
...
to_x = 0 # x좌표 기준으로 집게 이미지를 이동시킬 값 저장변수
caught_gemstone = None #집게를 뻗어서 잡은 보석정보
#속도 변수
move_speed = 12
return_speed = 20
#방향 변수
    ...
    if not caught_gemstone: #잡힌 보석이 없다면 충돌 체크
        for gemstone in gemstone_group:
            if claw.rect.colliderect(gemstone.rect): 
                ...
                to_x = -gemstone.speed # 잡힌 보석의 속도에 - 한값을 이동 속도로 설정
                break
